// pages/api/catalog/presets.js
// GET /api/catalog/presets — returns all model presets
...
export default function handler(req, res) {
  ...
  try {
    const { MODEL_PRESETS } = require('../../../src/data/modelPresets')
    ...
    const presets = Object.entries(MODEL_PRESETS || {}).map(([modelCode, preset]) => ({
      ...
      componentCount: (preset.components || []).length,
      addonCount: (preset.addons || []).length,
